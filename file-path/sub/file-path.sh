...
script_folder_="$(cd "$(dirname "$0")";pwd)";
echo "script_folder_ = ${script_folder_}"

script_folder_="$(cd "$(dirname "${BASH_SOURCE[0]}")" >/dev/null 2>&1 && pwd)";
echo "script_folder_ = ${script_folder_}"
...
# use string function
script_parent_folder_=${script_folder_path%/*};
...
execution_folder_=$(cd "$(dirname "$BASH_FOLDER")";pwd);
echo "execution_folder_path = ${execution_folder_path}"

# where you execute the bash
execution_folder_="$(pwd)"; echo "execution_folder_path = ${execution_folder_path}"
